...
export class BorderParserNew implements Parser {
    ...
    public parse(worldMap: string): WorldItem[] {
        ...
        borders.forEach(border => {
            if (border.vertices.filter(vertex => !usedVertices.has(vertex)).length > 0) {
                worldItems.push(this.createWorldItemFromBorder(border, graph));
            }
    ...
    private createWorldItemFromBorder(border: Border, graph: CharGraph) {
        ...
        const rect = this.createRectangleFromBorder(border, graph);
    ...
    private createRectangleFromBorder(border: Border, graph: CharGraph) {
