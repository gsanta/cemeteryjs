import { GeometryService, Line, Point, Polygon, StripeView, Shape } from '@nightshifts.inc/geometry';
...
export class SegmentBordersModifierNew  implements Modifier {
    ...
    private segmentOriginalBorderIntoPieces(originalBorderItem: WorldItem, segments: Segment[]): WorldItem[] {
        ...
        segments.map(segment => {
            ...
            clone.worldMapPositions = this.polygonToPoints(clone.dimensions)
    ...
    private polygonToPoints(polygon: Shape): Point[] {
        ...
        for (let i = topLeft[0]; i < bottomRight[0]; i++) {
            for (let j = topLeft[1]; j < bottomRight[1]; j++) {
                points.push(this.services.geometryService.factory.point(i, j));
            }
        }
