...
export class ConvertBorderPolyToLineModifier implements Modifier {
        ...
        const newRoomDimensions = rooms.map(room => {
            ...
            try {
                for (let i = 0; i < borderRoomSidePairs.length; i++) {
                    ...
                    const segmentPair1 = minBy(variations, (a, b) => a[0].getBoundingCenter().distanceTo(a[1].getBoundingCenter()) - b[0].getBoundingCenter().distanceTo(b[1].getBoundingCenter()));
                    const segmentPair2 = maxBy(variations, (a, b) => a[0].getBoundingCenter().distanceTo(a[1].getBoundingCenter()) - b[0].getBoundingCenter().distanceTo(b[1].getBoundingCenter()));
    ...
    private pairBordersToRoomSides(borderPolygons: [Shape, number][], roomSides: Segment[]): { border: Shape, roomSide: Segment}[]  {
        ...
        borderPolygons.forEach(item => {
            ...
            const closestIndexedEdge = minBy(
                indexedEdgesWithCorrectAngle,
                (a, b) => a[0].getBoundingCenter().distanceTo(item[0].getBoundingCenter()) - b[0].getBoundingCenter().distanceTo(item[0].getBoundingCenter())
            );
