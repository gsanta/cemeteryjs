...
describe('GameObjectParser', () => {
    describe('parse', () => {
        it('creates a GameObject for every furniture in the input map', () => {
            const file = fs.readFileSync(__dirname + '/../assets/test/test1.gwm', 'utf8');
            const gameObjectParser = new GameObjectParser();

            const {furnishing} = gameObjectParser.parse(file)
            expect(furnishing.length).to.equal(7);
            expect(furnishing[0]).to.eql(new GameObject('W', new Rectangle(1, 2, 1, 6), 'wall'), 'gameObject[0] is not correct');
            expect(furnishing[1]).to.eql(new GameObject('W', new Rectangle(8, 2, 1, 6), 'wall'), 'gameObject[1] is not correct');
            expect(furnishing[2]).to.eql(new GameObject('W', new Rectangle(2, 2, 2, 2), 'wall'), 'gameObject[2] is not correct');
            expect(furnishing[3]).to.eql(new GameObject('W', new Rectangle(2, 6, 6, 2), 'wall'), 'gameObject[3] is not correct');
            expect(furnishing[4]).to.eql(new GameObject('W', new Rectangle(6, 2, 2, 2), 'wall'), 'gameObject[4] is not correct');
            expect(furnishing[5]).to.eql(new GameObject('I', new Rectangle(4, 2, 2, 2), 'window'), 'gameObject[5] is not correct');
        });
        ...
        it('attaches the additional data to vertices, if present TEST CASE 1', () => {
            ...
            const {furnishing} = gameObjectParser.parse(file)
            expect(furnishing[0].additionalData).to.eql({
                angle: 90,
                axis: {
                    x: 4, y: 1
                },
        ...
        it('attaches the additional data to vertices for rectangular GameObjects', () => {
            ...
            const gameObjectParser = new GameObjectParser();
            const {furnishing} = gameObjectParser.parse(map)
            expect(furnishing[0].additionalData).to.eql({
                "pos": {
                    "x": 4,
                    "y": 1
        ...
        it('converts the additional data if conversion function provided', () => {
            ...
            const gameObjectParser = new GameObjectParser();
            const {furnishing} = gameObjectParser.parse(map, conversionFunction)
            expect(furnishing[0].additionalData).to.eql(                        {
