...
export class NodeFactory {
    ...
    nodeByType: Map<string, NodeModel | NodeGroupConfig> = new Map(); 
    nodeCreators: Map<string, () => NodeModel> = new Map();
    ...
    // placeNodeGroup(centerPoint: Point, nodesWithRelativePosition: [NodeModel, Point][]) {
    //     const position = this.registry.services.pointer.pointer.curr;

    //     nodesWithRelativePosition.forEach(([node, relativePosition]) => {
    //         const topLeft = centerPoint.clone().add(relativePosition);
    ...
    //         this.createConnections(this.nodeGroups.get(type), nodeViews);node: NodeView, 
    ...
    registerNodeCreator(creator: () => NodeModel) {
        const nodeObject = creator();
        this.nodeByType.set(nodeObject.type, nodeObject);
    }

    registerNodeGroup(nodeGroupConfig: NodeGroupConfig) {
        this.nodeByType.set(nodeGroupConfig.type, nodeGroupConfig);
    }
    ...
    //         const offset = new Point        const position = this.registry.services.pointer.pointer.curr;
    ...
    //             this.registry.stores.nodeStore.addNode(nodeView);
    //             return nodeView;
    //         });(node.relativeCoordInUnit.x * 20, node.relativeCoordInUnit.y * 20); 
    //         const delta = node.relativeCoordInUnit
    //             .mul(defaultNodeViewConfig.width, defaultNodeViewConfig.height)
    //             .add(offset);
    //         const topLeft = position.clone().add(delta);
