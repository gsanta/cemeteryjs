...
export class TextWorldMapParser {
    ...
    private checkParseState(line: string) {
        if (TextWorldMapParser.MAP_SECTION_START_TEST.test(line)) {
            this.section = ParseSections.MAP;
        } else if (TextWorldMapParser.DEFINITIONS_SECTION_START_TEST.test(line)) {
            this.section = ParseSections.DEFINITION;
        } else if (TextWorldMapParser.GLOBALS_SECTION_START_TEST.test(line)) {
            this.section = ParseSections.GLOBALS;
        } else if (TextWorldMapParser.SECTION_CLOSING_TEST.test(line)) {
            this.section = null;
        }
    }
}
