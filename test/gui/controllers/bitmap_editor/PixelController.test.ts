...
it ('Add new pixels based on their canvas position', () => {
    ...
    canvasController.pixelModel.addPixel(new Point(5, 5), 'wall', false);
    expect(canvasController.pixelModel.bitMap.has(0)).toBeTruthy();
    expect(canvasController.pixelModel.bitMap.get(0)).toEqual({index: 0, type: 'wall', isPreview: false});

    canvasController.pixelModel.addPixel(new Point(10, 9), 'door', false);
    expect(canvasController.pixelModel.bitMap.has(1)).toBeTruthy();
    expect(canvasController.pixelModel.bitMap.get(1)).toEqual({ index: 1, type: 'door', isPreview: false});

    canvasController.pixelModel.addPixel(new Point(10, 10), 'window', false);
    expect(canvasController.pixelModel.bitMap.has(11)).toBeTruthy();
    expect(canvasController.pixelModel.bitMap.get(11)).toEqual({index: 11, type: 'window', isPreview: false});

    canvasController.pixelModel.addPixel(new Point(31, 15), 'bed', false);
    expect(canvasController.pixelModel.bitMap.has(13)).toBeTruthy();
    expect(canvasController.pixelModel.bitMap.get(13)).toEqual({index: 13, type: 'bed', isPreview: false});
});