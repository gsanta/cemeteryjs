...
it ('Add new pixels based on their canvas position', () => {
    ...
    expect(canvasController.pixelModel.bitMap.get(0)).toEqual([{index: 0, type: 'wall', isPreview: false, layer: 0}]);
    ...
    expect(canvasController.pixelModel.bitMap.get(1)).toEqual([{ index: 1, type: 'door', isPreview: false, layer: 0}]);
    ...
    expect(canvasController.pixelModel.bitMap.get(11)).toEqual([{index: 11, type: 'window', isPreview: false, layer: 0}]);
    ...
    expect(canvasController.pixelModel.bitMap.get(13)).toEqual([{index: 13, type: 'bed', isPreview: false, layer: 0}]);
});