...
it ('Write an svg from the internal worldmap representation', () => {
    ...
    const {gameObjectTemplates, globalConfig} = new SvgConfigReader().read(inputWorldMap);

    const outputWorldMap = new SvgWorldMapWriter().write(graph, gameObjectTemplates, globalConfig);
